...
static const struct spdk_json_object_decoder rpc_create_xnvme_decoders[] = {
	...
	{"fixed_bufs", offsetof(struct rpc_create_xnvme, fixed_bufs), spdk_json_decode_bool, true},
};
		      ...
		      const struct spdk_json_val *params)
{
	...
	bdev = create_xnvme_bdev(req.name, req.filename, req.io_mechanism, req.conserve_cpu, req.fixed_bufs);
	if (!bdev) {
